...
// set up timestamp for server creation
const serverCreationTime = moment();
...
// Add route for time since last crash
server.get('/meta/runtime', (req, res) => {
  ...
  if (
    ...
    validMeasurmements.indexOf(req.query.measurement) >= 0
  )
    measurement = req.query.measurement;
  res.send({
    timeSinceLastReboot: `${moment().diff(
      serverCreationTime,
      measurement
    )} ${measurement}`,
  });
